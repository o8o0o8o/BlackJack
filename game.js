...
function deal(str) {
  switch (str) {
    case "p":
      ...
      getSuit(playerHand[playerHand.length - 1])(
        cardRang(playerHand[playerHand.length - 1]),
        "playerhand"
      );
      break;
    case "d":
      dealerHand.push(getCard());
      getSuit(dealerHand[dealerHand.length - 1])(
        cardRang(dealerHand[dealerHand.length - 1]),
        "dealerhand"
      );
      break;
  }
...
function changeState() {
  if (state === "end") {
    ...
    document
      .getElementById("dealerhand")
      .parentNode.removeChild(document.getElementById("dealerhand"));
    document
      .getElementById("playerhand")
      .parentNode.removeChild(document.getElementById("playerhand"));
  }
}
...
function getStat() {
  document.getElementById(
    "statistics"
  ).innerHTML = `Dealer Won  ${statistics.dealerWon}  Player Won  ${statistics.playerWon}`;
}

function showScore() {
  document.getElementById(
    "score"
  ).innerHTML = `GameOver:  dealerScore ${dealerScore}  playerScore ${playerScore}  State ${state}`;
}
